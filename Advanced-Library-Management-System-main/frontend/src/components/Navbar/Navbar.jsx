...
const Navbar = () => {
  ...
  const handleReload = () => {
    setLoading(true);
    setTimeout(() => setLoading(false), 2000); // 2s de chargement après clic
  };

 return (
    <nav className={`bg-white shadow-lg w-full z-50 transition-all ${isSticky ? "fixed top-0 left-0 shadow-md" : "relative"}`}>
      <div className="container flex justify-between items-center py-3 px-4 md:px-6">
        
        {/* Logo */}
        <Link to="/" onClick={() => { handleReload(); setIsMobileMenuOpen(false); }} className="flex items-center gap-2 text-2xl font-bold text-zinc-800">
        ...
        <div className="hidden md:flex flex-grow justify-center">
          <div className="relative flex items-center w-full max-w-md border border-gray-300 rounded-lg overflow-hidden shadow-sm">
            ...
            <input type="text" placeholder="Search..." className="w-full px-3 py-2 pl-10 text-sm outline-none focus:ring-2 focus:ring-primary rounded-lg" />
          </div>
        </div>
        ...
        <div className="hidden sm:flex items-center gap-6">
          <ul className="flex items-center space-x-6">
            {Menu.map(menu => (
              <li key={menu.id}>
                <Link to={menu.link} className="py-2 hover:text-primary transition">{menu.name}</Link>
              </li>
            ))}
            ...
            <li className="relative group">
              <button className="flex items-center gap-2">
                Categories <FaCaretDown className="transition group-hover:-rotate-180" />
              </button>
          ...
          {/* Icône de connexion */}
          <Link to="/login">
            <PersonIcon className="cursor-pointer text-gray-600 hover:text-blue-600 transition" />
          </Link>
        </div>
      ...
      {/* Menu mobile */}
      {isMobileMenuOpen && (
        <div className="sm:hidden bg-white transition-all">
                ...
                <ul>
                  {categoriesList.map(categorie => (
                    <li key={categorie.id} className="p-2 hover:bg-gray-200 rounded-md">
                      <Link to={`/${categorie.name}`} onClick={() => setIsMobileMenuOpen(false)}>{categorie.name}</Link>
                    </li>
                  ))}
                </ul>
              </div>
